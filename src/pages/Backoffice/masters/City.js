  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!ignore) { setErr(e?.message || "Failed to load cities."); setRows([]); setTotal(0); }
      } finally {
      ...
      <div className="res-wrap">
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add City</button>
          </div>
        </div>
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/cities/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/cities", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save city.");
    } finally {
      setSaving(false);
    }
  };
