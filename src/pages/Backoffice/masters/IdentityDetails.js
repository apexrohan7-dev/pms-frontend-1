...
export default function IdentityDetails() {
  ...
  const [propertyCode, setPropertyCode] = useState(
    (localStorage.getItem(LS_KEY) || "").toUpperCase()
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // default selection (first property) if none saved
        if (!propertyCode && list.length) {
          const next = (list[0].code || "").toUpperCase();
          setPropertyCode(next);
          localStorage.setItem(LS_KEY, next);
        }
      } catch {
        // ignore: page can still work without dropdown populated
      }
    })();
    return () => { ignore = true; };
  }, []); // eslint-disable-line

  // 2) load identity list (filtered by propertyCode)
  useEffect(() => {
    let ignore = false;
    (async () => {
      setLoading(true);
      ...
      try {
        const params = new URLSearchParams({
          ...
          page,
          limit,
          propertyCode: propertyCode || "", // empty -> global
        });
        const res = await apiFetch(`/api/identity-details?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load identity details.");
          setRows([]); setTotal(0);
        }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Identity Details</h2>
        ...
        <div className="panel">
          ...
          <div className="panel-b">
            ...
            {!propertyCode && properties.length > 0 && (
              <Banner type="err">Please select a property to view data.</Banner>
            )}
      ...
      {showForm && (
        <IdentityFormModal
          ...
          onSaved={afterSave}
          // pass property context + list for the form
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/identity-details/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function IdentityFormModal({ initial, onClose, onSaved, currentPropertyCode, properties }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/identity-details/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/identity-details", {
      ...
      setErr(e2?.message || "Failed to save identity.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal title={isEdit ? "Edit Identity" : "Create Identity"} onClose={onClose}>
      {err && <Banner type="err">{err}</Banner>}
