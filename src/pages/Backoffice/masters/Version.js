// src/pages/Backoffice/masters/Version.js
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/versions?${params.toString()}`, { auth: true });
        ...
        if (!ignore) { setErr(e?.message || "Failed to load versions."); setRows([]); setTotal(0); }
      } finally {
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Version</button>
          </div>
        </div>
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.deliveryAppVersion || "—"}</td>
                        <td>{r.qrVersion || "—"}</td>
                        <td>{r.tibooVersion || "—"}</td>
                        <td>{r.versionName || "—"}</td>
                        <td>{r.updateStatus || "—"}</td>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            await apiFetch(`/api/versions/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
...
/* ---------- Form Modal ---------- */
function VersionFormModal({ initial, onClose, onSaved }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/versions/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/versions", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
      setOk("Saved.");
      onSaved(saved);
    } catch (e2) {
      setErr(e2?.message || "Failed to save version.");
    } finally {
      setSaving(false);
    }
  };
