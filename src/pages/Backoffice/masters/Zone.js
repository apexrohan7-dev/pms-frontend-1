// src/pages/Backoffice/masters/Zone.js
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!ignore) { setErr(e?.message || "Failed to load zones."); setRows([]); setTotal(0); }
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        <div className="res-topbar">
          <h2 style={{ margin: 0 }}>Zone Master</h2>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Zone</button>
          </div>
        </div>
                  ...
                  {dataToRender?.map(r => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td>{r.zoneName || "—"}</td>
                        <td>{r.value !== undefined && r.value !== null ? r.value : "—"}</td>
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/zones/${id}`, { method: "PATCH", auth: true, body: JSON.stringify(payload) });
      } else {
        saved = await apiFetch("/api/zones", { method: "POST", auth: true, body: JSON.stringify(payload) });
      }
