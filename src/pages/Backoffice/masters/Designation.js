// src/pages/Backoffice/masters/Designation.js
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/designations?${params.toString()}`, { auth: true });
        ...
        if (!ignore) {
          setErr(e?.message || "Failed to load designations.");
          setRows([]); setTotal(0);
        }
      } finally {
    ...
    <div className="page" style={{ display: "grid", gridTemplateColumns: "auto 1fr" }}>
      ...
      <div className="res-wrap">
        ...
        <div className="res-topbar">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn" onClick={openCreate}>+ Add Designation</button>
          </div>
        </div>
      ...
      {showDelete && (
        <ConfirmModal
          ...
          message={`Delete designation "${toDelete?.title}"? This cannot be undone.`}
          confirmText="Delete"
          onClose={() => { setShowDelete(false); setToDelete(null); }}
          onConfirm={async () => {
            const id = toDelete?._id || toDelete?.id;
            await apiFetch(`/api/designations/${id}`, { method: "DELETE", auth: true });
            afterDelete(id);
          }}
        />
      )}
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        saved = await apiFetch(`/api/designations/${id}`, {
          method: "PATCH", auth: true, body: JSON.stringify(payload),
        });
      } else {
        saved = await apiFetch("/api/designations", {
      ...
      setErr(e2?.message || "Failed to save designation.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <Row>
          <Field label="Property" required>
            ...
            >
              <option value="">{propsLoading ? "Loading properties…" : "Select a property"}</option>
              {propsList.map(p => {
                const id = p._id || p.id;
